        ...
        for i in range(len(self.fuse_layers)):
            ...
            for j in range(1, self.num_branches):
                if i == j:
                    ...
                    # 双线性上采样
                    y = y + F.interpolate(
                        self.fuse_layers[i][j](x[j]), #表示输入
                        size=[height_output, width_output], #表示采样大小
...
class HighResolutionNet(nn.Module):
    ...
    def _make_stage(self, layer_config, num_inchannels,
        ...
        modules = []
        for i in range(num_modules): #num_modules 一直是1,所以不太清楚modules代表啥
