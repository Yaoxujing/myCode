...
class YuvDct(nn.Module):
    def __init__(self, device, yuv_keep_weights = (25, 9, 9)):
        super(YuvDct, self).__init__()
        self.device = device
        # 维度（64,1,8,8） #图像必须是8*8的
    ...
    def create_mask(self, requested_shape):
        if self.jpeg_mask is None or requested_shape > self.jpeg_mask.shape[1:]:
            self.jpeg_mask = torch.empty((3,) + requested_shape, device=self.device) #torch.Size([3, 1000, 1000])
            for channel, weights_to_keep in enumerate(self.yuv_keep_weighs):  #channel: 0,1,2  weights_to_keep : 25,9,9
    ...
    def apply_conv(self, image, filter_type: str):
        ...
        for channel in range(image.shape[1]):
            image_yuv_ch = image[:, channel, :, :].unsqueeze_(1) #[n,1,h,w]
            image_conv = F.conv2d(image_yuv_ch, filters, stride=8) #torch.Size([16, n, h/8, w/8])
            image_conv = image_conv.permute(0, 2, 3, 1) #换成[16: 长 ：宽： n 是64吗]
            image_conv = image_conv.view(image_conv.shape[0], image_conv.shape[1], image_conv.shape[2], 8, 8)
            image_conv = image_conv.permute(0, 1, 3, 2, 4) #[16: 长 ：8：宽：8]
            ...
            image_conv.unsqueeze_(1) #[16, 1, 长*8 , 宽*8]
    ...
    def forward(self, image_input):
        ...
        #向下向右padding成8的整数倍
        noised_image = image_input[0] #这个第0维信息是指？ 
        #[10,3,256,256] 
        ...
        #返回到原来的大小
        image_input[0] = image_dct[:, :, :image_dct.shape[2]-pad_height, :image_dct.shape[3]-pad_width].clone()
        return image_input